...
\begin{lstlisting}[title=Pseudoinstrucción Final]
lw $1, Imm(Rt)
sbr $1, Rs, $0
nop
sw	$1, Imm(Rt)
...
La instrucción nueva \textbf{sbr} requirió la modificación al \textbf{Data Path}:
...
\item Se agrego un Fork que obtiene el ReadData2 del EX/MEM y lo envia, además de donde enviaba antes, a un nuevo distribuidor donde se toman los bits 0 a 7 (el byte menos significativo)
\item Se agrego un Concatenador que toma de esos 2 distribuidores los bytes quedando un word nuevamente.
\item Se agrego una línea de control llamada WriteByte.
...
\begin{enumerate}
\item Agregar la instrucción sbr, con código de operación 44.
\item Agregar la pseudo-instrucción sb con las llamadas a las instrucciones.
\item Agregar a la unidad de control que la operación sbr utiliza las líneas de control ALUSrc, RegDst, RegWrite, MemToReg y la nueva línea WriteByte.
\item Se agrego la operación 3 de la ALU que le dice que utilice el OR para ser mas ordenados.
\end{enumerate}
...
\item En el Pipeline el riesgo de datos para el bgezal se salvo con Forwarding gracias a que las instrucciones básicas ya tenian implementado dicho mecanismo y que el DrMIPS acepta la unidad de Forwarding.
\item El sb trajo tanta complejidad que la catedra pidió que se haga con una mezcla de una pseudio-instrucción y una instrucción nueva interna, esto hizo que tengamos que volver a pensar la instrucción, dado que lo que pensamos para la primera entrega no servía y volver a modificar el datapath, también trajo un riesgo de datos que tuvimos que salvar con una instrucción nop al no saber manejar correctamente la unidad de Forwarding teniendo que perder un ciclo del pipeline.
